...
export const DiscountCard: React.FC<DiscountCardProps> = ({
    ...
    const isActiveDiscount = data.dataType.toUpperCase() === "BIT" ? data.bitValue === true : data.enabled;
    ...
    const handleViewDocument = async (documentGuid?: string) => {
        ...
        setIsViewing(true);
        try {
            const res = await getDocumentBlobUrl(documentGuid, locale);
            setViewerData({
                isOpen: true,
                url: res.url,
                name: label || "Discount Document"
            });
        } catch (error: unknown) {
    ...
    return (
        <>
            <div
                className={`relative border rounded-xl p-4 transition-all duration-300 ease-out transform group ${isActiveDiscount
            ...
            >
                <div className="flex justify-between items-center mb-4">
                    <span className={`text-xs font-bold ${isActiveDiscount ? "text-blue-800" : "text-gray-500"}`}>
                        {label}
                    </span>

                    <div className="flex items-center gap-2">
                        {isActiveDiscount && data.documentGuid ? (
                            <button
                                type="button"
                ...
                <div className={`col-span-2 mt-2 pt-3 border-t ${isActiveDiscount ? "border-blue-200" : "border-gray-100"}`}>
                    <div className="flex items-center justify-between gap-2">
                        <div className="flex-1 overflow-hidden">
                            <Label className={`text-[10px] font-semibold block mb-1 ${isActiveDiscount ? "text-blue-800" : "text-gray-500"}`}>
                                {t("discount.uploadDocument")}
                            </Label>
                            <div className="flex items-center gap-2">
                                <label
                                    className={`flex-1 flex items-center justify-center h-8 gap-2 border-2 border-dashed rounded-md transition-all cursor-pointer text-[10px] px-2 ${isActiveDiscount
                                ...
                                >
                                    {data.isUploading ? (
                                        ...
                                        <Upload size={14} className={isActiveDiscount ? "text-blue-600" : "text-gray-300"} />
                                    )}
                                    ...
                                    <input
                                        type="file"
                                        className="hidden"
                                        disabled={!isActiveDiscount || data.isUploading || isViewing}
                                        onChange={(e) => {
                        ...
                        <div className="pt-[18px]">
                            <Button
                                size="sm"
                                disabled={!isActiveDiscount || !data.documentGuid || data.isUploading || isViewing}
                                isLoading={isViewing}
                                icon={Eye}
